...
async def handle_change_list(
    rss: rss_class.Rss, key_to_change: str, value_to_change: str, group_id: int, guild_channel_id: str
):
...
async def handle_rss_change(bot: Bot, event: Event, state: dict):
    ...
    if group_id:
        if re.search(" (qq|qun|channel)=", change_info):
            await RSS_CHANGE.send("❌ 禁止在群组中修改 QQ号 / 群号 / 频道号！如要取消订阅请使用 deldy 命令！")
            return
        rss_list = [rss for rss in rss_list if str(group_id) in rss.group_id]

    if guild_channel_id:
        if re.search(" (qq|qun|channel)=", change_info):
            await RSS_CHANGE.send("❌ 禁止在子频道中修改 QQ号 / 群号 / 频道号！如要取消订阅请使用 deldy 命令！")
            return
